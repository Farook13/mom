...
async def send_for_index(bot, message):
    ...
    try:
        ...
        return await message.reply('Make sure I am an admin in the channel, if it’s private.')

    if k.empty:
        return await message.reply('This might be a group and I’m not an admin.')
    ...
    await message.reply('Thank you for your contribution. Our moderators will review and approve it shortly.')
...
async def set_skip_number(bot, message):
    if ' ' in message.text:
        ...
        try:
            ...
            await message.reply(f"Skip count set to {skip}")
        except ValueError:
            ...
            async for message in bot.iter_messages(chat, lst_msg_id, temp.CURRENT):
                if temp.CANCEL:
                    try:
                        ...
                        await msg.edit("Indexing cancelled (retry after wait)")
                    break
                ...
                if current % 20 == 0:
                    try:
                        await msg.edit_text(
                            text=f"Fetching: <code>{current}</code>\nSaved: <code>{total_files}</code>\nDuplicates: <code>{duplicate}</code>\n"
